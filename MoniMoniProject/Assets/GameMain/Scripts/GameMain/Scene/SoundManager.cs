...
// 音管理クラス http://zyyxlabo.blogspot.jp/2013/03/unitysoundmanager-ver.html
public class SoundManager : MonoBehaviour
{
    ...
    {
        if (0 > index || BGM.Length <= index) { return; }
        // 同じBGMの場合は何もしない  
        if (BGMsource.clip == BGM[index]) { return; }
        BGMsource.Stop(); BGMsource.clip = BGM[index]; BGMsource.Play();
    }
    // BGM停止  
    public void StopBGM()
    {
    ...
    public void PlaySE(int index)
    {
        ...
        foreach (AudioSource source in SEsources)
        {
            ...
            {
                source.clip = SE[index]; source.Play(); return;
            }
        }
    }
    // SE停止  
    ...
    public void PlayVoice(int index)
    {
        ...
        foreach (AudioSource source in VoiceSources)
        {
            if (false == source.isPlaying)
            {
                source.clip = Voice[index]; source.Play(); return;
            }
        }
    }
    // 音声停止  
